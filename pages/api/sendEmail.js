...
const sendEmail = async (message, files) => {
  const tableContent = `
    ...
    <tr>
        <td>Phone</td>
        <td>${
          message.phone != undefined || message.phone != null
            ? message.phone
            : ''
        }</td>
    </tr>
    ...
    <tr>
        <td>role</td>
        <td>${
          message.role != undefined || message.role != null ? message.role : ''
        }</td>
    </tr>
</table>
...
const handler = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ success: false, message: 'Error processing form submission.' });
  }
};
