...
export default function Business_benefits({ apiData }) {
  ...
  return data?.Benefits != null ? (
    <div
      className="w-full bg-[#bbe6f0] mt-[30px]"
        ...
        <div className="flex mt-[15px] justify-between gap-[20px]">
          <div className="w-[450px] md:flex hidden">
            <div className="w-[450px] h-full">
              <div
                className="w-[450px] h-full rounded-[13px]  flex flex-col p-[30px] relative justify-between"
                style={{
                  background:
                    'linear-gradient(45deg,#000850,#000320),radial-gradient(100% 225% at 100% 0,#ff6928 0,#000 100%),linear-gradient(225deg,#ff7a00,#000),linear-gradient(135deg,#cdffeb 10%,#cdffeb 35%,#009f9d 0,#009f9d 60%,#07456f 0,#07456f 67%,#0f0a3c 0,#0f0a3c)',
                  backgroundBlendMode: 'screen,overlay,hard-light,normal'
                }}
              >
                <div
                  className="relative flex-col px-2 py-4 h-full flex"
                  style={{
                    backgroundImage: `url(${
                      data.Benefits.image?.data
                        ? process.env.NEXT_PUBLIC_mainurl +
                          data.Benefits.image.data.attributes.url
                        : ''
                    })`,
                ...
                >
                  ...
                  <div className="w-full flex items-center z-10 mt-auto">
                    <Link href={`/${data.Benefits.url}`} className="ml-auto">
                      {' '}
                      <button
                        className="uppercase text-[20px] w-[160px] hover:bg-[white] hover:text-[#fa5252] text-[white] px-3 py-2 border-solid  rounded-[25px] font-roboto z-10 transition delay-[100ms] duration-200"
                        style={{ borderColor: 'rgb(255, 122, 0)' }}
                      >
                        Read more +
                      </button>
                    </Link>
